...
Shopify.onCartUpdate = function(cart) {
  Shopify.cartUpdateInfo(cart, '.cart_list');
};
    ...
    if (cart_cell.length) {
      ...
      jQuery.each(cart, function(key, value) {
        if (key === 'items') {
          ...
          if (value.length) {
            jQuery(".cart_box.dropdown-menu").css({"display": "block"});
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li><a href="javascript:void(0);" class="item_remove" onclick="Shopify.removeItem(' + item.variant_id + ')"><i class="ion-close"></i></a><a href="' + item.url + '"><img src="' + item.image + '" alt="cart_thumb1">' + item.title + '</a><span class="cart_quantity">' + item.quantity + ' x <span class="cart_amount"> <span class="price_symbole">' + Shopify.formatMoney(item.price, formatMoney) + '</span></span></span></li>').appendTo(table);
            });
          }
          else {
            jQuery(".cart_box.dropdown-menu").css({"display": "none"});
